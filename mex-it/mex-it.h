...
namespace mex_binding {
    ...
    struct enable_if_cond : public std::enable_if<Cond::value, T> {
    };
    template<class Cond, class T = void>
    struct disable_if_cond : public disable_if<Cond::value, T> {
    };
    ...
    struct function_args_tuple<N, T> {
        typedef typename std::tuple<typename std::decay<T>::type> root_type;
    };

    template<int N, typename T, typename ...InTypes>
    struct function_args_tuple<N, T, InTypes...> {
        typedef typename append_to_type_seq<typename std::decay<T>::type, typename function_args_tuple<
                N - 1, InTypes...>::root_type>::type root_type;
    };
    // ----------------------------------------------------------------------------------------
    ...
    template<typename Func>
    inline apply_unpack_t<Func> auto_unpack(Func &&func) {
...
// for the mex process using Variadic Templates, etc
    ...
    template<typename T>
    struct is_eigen_vector : public default_is_kind_value {
    };
    ...
    template<typename T, typename helper = void>
    struct is_eigen_matrix : public default_is_kind_value {
    };
    ...
    struct is_array_type {
        const static bool value = is_std_vector<T>::value || is_array<T>::value
#ifdef EIGEN_MAJOR_VERSION
    ...
    template<typename... Args>
    struct get_num_inputs;

    template<typename T>
    struct get_num_inputs<T> {
        enum {
            value = is_input_type<T>::value
        };
    };

    template<typename T, typename... Args>
    struct get_num_inputs<T, Args...> {
        enum {
            value = is_input_type<T>::value + get_num_inputs<Args...>::value
        };
    };
    ...
    template<typename... Args>
    struct get_num_outputs;

    template<typename T>
    struct get_num_outputs<T> {
        enum {
            value = is_output_type<T>::value
        };
    };

    template<typename T, typename... Args>
    struct get_num_outputs<T, Args...> {
        enum {
            value = is_output_type<T>::value + get_num_outputs<Args...>::value
        };
    };
    ...
    template<typename T>
    struct inner_type<T, typename enable_if_cond<is_std_vector<T>>::type> {
...
// Copyright (c) 2014 Tony Kirke
    ...
    template<typename T>
    void populate_to_eigen_mat(const long arg_idx, T &m, const mxArray *src, long nc, long nr) {
        // generic - should never happen since below specialization should be used for Eigen::Matrix
        std::ostringstream sout;
        sout << "mex_function has some bug in it related to processing input argument " << arg_idx + 1 << " on line "
             << __LINE__ << "\n";
    ...
    template<typename T>
    void populate_to_vector(const long arg_idx, T &m, const mxArray *src, long nc) {
    ...
    template<>
    void populate_to_vector(const long arg_idx, std::vector<double> &m,
    ...
    template<typename T>
    void assign_function_handle(const long arg_idx, T &, const mxArray *) {
        std::ostringstream sout;
        sout << "mex_function has some bug in it related to processing input argument " << arg_idx + 1 << " on line "
             << __LINE__ << "\n";
        mexErrMsgIdAndTxt("mex_function:validate_and_populate_arg", sout.str().c_str());
    ...
    template<typename T>
    typename enable_if_cond<is_matrix<T>>::type assign_to_matlab(mxArray *&plhs, const T &item) {
    ...
    template<typename T>
    typename enable_if_cond<is_eigen_matrix<T>>::type assign_to_matlab(mxArray *&plhs, const T &item) {
        typedef typename is_eigen_matrix<T>::type type;
        type *mat = 0;
        if (std::is_same<double, type>::value) {
    ...
    template<typename T>
    typename disable_if<is_eigen_matrix<T>::value || is_matrix<T>::value || is_array_type<T>::value ||
                        std::is_same<T, function_handle>::value>::type
    assign_to_matlab(mxArray *&plhs, const T &item) {
        plhs = mxCreateDoubleScalar(item);
    }

    template<typename T>
    typename enable_if_cond<is_array_type<T>>::type assign_to_matlab(mxArray *&plhs, const T &item) {
    ...
    template<typename T>
    void validate_scalar_type(long arg_idx, const mxArray *prhs) {
        std::ostringstream sout;
        if (std::is_same<T, uint8_t>::value && (mxGetClassID(prhs) != mxUINT8_CLASS)) {
            sout << " argument " << arg_idx + 1 << " types don't match!, input is " << mxGetClassName(prhs)
                 << " expected type uint8_t\n";
    ...
    template<typename T>
    void validate_and_populate_arg(long arg_idx, const mxArray *prhs, T &arg) {
        if (std::is_arithmetic<T>::value || std::is_same<T, bool>::value) {
            ...
            if (std::is_same<type, double>::value) {
                if (!mxIsDouble(prhs) || mxIsComplex(prhs)) {
                    ok = false;
                    sout << " argument " << arg_idx + 1 << " must be a vector of doubles , not " << mxGetClassName(prhs)
                         << "s\n";
                }
            } else if (std::is_same<type, float>::value) {
                if (!mxIsSingle(prhs) || mxIsComplex(prhs)) {
                    ok = false;
                    sout << " argument " << arg_idx + 1 << " must be a vector of single/float, not "
                         << mxGetClassName(prhs) << "s\n";
                }
            } else if (std::is_same<type, bool>::value) {
                if (!mxIsLogical(prhs)) {
                    ok = false;
                    sout << " argument " << arg_idx + 1 << " must be a vector of logical elements, not "
                         << mxGetClassName(prhs) << "s\n";
                }
            } else if (std::is_same<type, uint8_t>::value) {
                if (!mxIsUint8(prhs) || mxIsComplex(prhs)) {
                    ok = false;
                    sout << " argument " << arg_idx + 1 << " must be a vector of uint8, not " << mxGetClassName(prhs)
                         << "s\n";
                }
            } else if (std::is_same<type, int8_t>::value) {
                if (!mxIsInt8(prhs) || mxIsComplex(prhs)) {
                    ok = false;
                    sout << " argument " << arg_idx + 1 << " must be a vector of int8, not " << mxGetClassName(prhs)
                         << "s\n";
                }
                ...
                if (!mxIsInt16(prhs) || mxIsComplex(prhs)) {
                    ok = false;
                    sout << " argument " << arg_idx + 1 << " must be a vector of int16, not " << mxGetClassName(prhs)
                         << "s\n";
                }
                ...
                if (!mxIsUint16(prhs) || mxIsComplex(prhs)) {
                    ok = false;
                    sout << " argument " << arg_idx + 1 << " must be a vector of uint16, not " << mxGetClassName(prhs)
                         << "s\n";
                }
                ...
                if (!mxIsInt32(prhs) || mxIsComplex(prhs)) {
                    ok = false;
                    sout << " argument " << arg_idx + 1 << " must be a vector of int32, not " << mxGetClassName(prhs)
                         << "s\n";
                }
                ...
                if (!mxIsUint32(prhs) || mxIsComplex(prhs)) {
                    ok = false;
                    sout << " argument " << arg_idx + 1 << " must be a vector of uint32, not " << mxGetClassName(prhs)
                         << "s\n";
                }
                ...
                if (!mxIsUint64(prhs) || mxIsComplex(prhs)) {
                    ok = false;
                    sout << " argument " << arg_idx + 1 << " must be a vector of uint64, not " << mxGetClassName(prhs)
                         << "s\n";
                }
                ...
                if (!mxIsInt64(prhs) || mxIsComplex(prhs)) {
                    ok = false;
                    sout << " argument " << arg_idx + 1 << " must be a vector of int64, not " << mxGetClassName(prhs)
                         << "s\n";
                }
            } else {
                ok = false;
                sout << " argument " << arg_idx + 1 << " must be a vector of a pod type, not " << mxGetClassName(prhs)
                     << "s\n";
            }
            if (ok) {
            ...
            if (std::is_same<type, double>::value) {
                if (!mxIsDouble(prhs) || mxIsComplex(prhs)) {
                    std::ostringstream sout;
                    sout << " argument " << arg_idx + 1 << " must be a matrix of doubles, not " << mxGetClassName(prhs)
                         << "s\n";
                    throw invalid_args_exception(sout.str());
                }
                ...
                if (!mxIsSingle(prhs) || mxIsComplex(prhs)) {
                    std::ostringstream sout;
                    sout << " argument " << arg_idx + 1 << " must be a matrix of single/float, not "
                         << mxGetClassName(prhs) << "s\n";
                    throw invalid_args_exception(sout.str());
                }
                populate_to_eigen_mat(arg_idx, arg, prhs, nc, nr);
                ...
                if (!mxIsLogical(prhs)) {
                    std::ostringstream sout;
                    sout << " argument " << arg_idx + 1 << " must be a matrix of logical elements, not "
                         << mxGetClassName(prhs) << "s\n";
                    throw invalid_args_exception(sout.str());
                }
                populate_to_eigen_mat(arg_idx, arg, prhs, nc, nr);
                ...
                if (!mxIsUint8(prhs) || mxIsComplex(prhs)) {
                    std::ostringstream sout;
                    sout << " argument " << arg_idx + 1 << " must be a matrix of uint8, not " << mxGetClassName(prhs)
                         << "s\n";
                    throw invalid_args_exception(sout.str());
                }
                populate_to_eigen_mat(arg_idx, arg, prhs, nc, nr);
                ...
                if (!mxIsInt8(prhs) || mxIsComplex(prhs)) {
                    std::ostringstream sout;
                    sout << " argument " << arg_idx + 1 << " must be a matrix of int8, not " << mxGetClassName(prhs)
                         << "s\n";
                    throw invalid_args_exception(sout.str());
                }
                populate_to_eigen_mat(arg_idx, arg, prhs, nc, nr);
                ...
                if (!mxIsInt16(prhs) || mxIsComplex(prhs)) {
                    std::ostringstream sout;
                    sout << " argument " << arg_idx + 1 << " must be a matrix of int16, not " << mxGetClassName(prhs)
                         << "s\n";
                    throw invalid_args_exception(sout.str());
                }
                populate_to_eigen_mat(arg_idx, arg, prhs, nc, nr);
                ...
                if (!mxIsUint16(prhs) || mxIsComplex(prhs)) {
                    std::ostringstream sout;
                    sout << " argument " << arg_idx + 1 << " must be a matrix of uint16, not " << mxGetClassName(prhs)
                         << "s\n";
                    throw invalid_args_exception(sout.str());
                }
                populate_to_eigen_mat(arg_idx, arg, prhs, nc, nr);
                ...
                if (!mxIsInt32(prhs) || mxIsComplex(prhs)) {
                    std::ostringstream sout;
                    sout << " argument " << arg_idx + 1 << " must be a matrix of int32, not " << mxGetClassName(prhs)
                         << "s\n";
                    throw invalid_args_exception(sout.str());
                }
                populate_to_eigen_mat(arg_idx, arg, prhs, nc, nr);
                ...
                if (!mxIsUint32(prhs) || mxIsComplex(prhs)) {
                    std::ostringstream sout;
                    sout << " argument " << arg_idx + 1 << " must be a matrix of uint32, not " << mxGetClassName(prhs)
                         << "s\n";
                    throw invalid_args_exception(sout.str());
                }
                populate_to_eigen_mat(arg_idx, arg, prhs, nc, nr);
                ...
                if (!mxIsUint64(prhs) || mxIsComplex(prhs)) {
                    std::ostringstream sout;
                    sout << " argument " << arg_idx + 1 << " must be a matrix of uint64, not " << mxGetClassName(prhs)
                         << "s\n";
                    throw invalid_args_exception(sout.str());
                }
                populate_to_eigen_mat(arg_idx, arg, prhs, nc, nr);
                ...
                if (!mxIsInt64(prhs) || mxIsComplex(prhs)) {
                    std::ostringstream sout;
                    sout << " argument " << arg_idx + 1 << " must be a matrix of int64, not " << mxGetClassName(prhs)
                         << "s\n";
                    throw invalid_args_exception(sout.str());
                }
                populate_to_eigen_mat(arg_idx, arg, prhs, nc, nr);
    ...
    template<typename funct, std::size_t I = 0, std::size_t N, typename Ts>
    ...
    void call_mex_function(const funct &f, int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
